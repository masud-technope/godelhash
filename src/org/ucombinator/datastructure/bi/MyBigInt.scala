...
package org.ucombinator.datastructure.utils.bi
...
object BigInt {
  ...
  def apply(i: Int): BigInt =
    if (minCached <= i && i <= maxCached) {
      ...
      var n = cache(offset)
      if (n eq null) { n = new BigInt(new GMP(i)); cache(offset) = n }
      n
    } else new BigInt(new GMP(i))
...
//    new BigInt(new GMP(bitlength, certainty, rnd.self))
...
//    new BigInt(new GMP(numbits, rnd.self))
 ...
 *  @version 1.0, 15/07/2003
 */
class BigInt(val bigInteger: GMP) extends ScalaNumber with ScalaNumericConversions with Serializable
{
  ...
  def /% (that: BigInt): (BigInt, BigInt) = {
    val dr = this.bigInteger.quotientAndRemainder(that.bigInteger)
  ...
  def >> (n: Int): BigInt = {
    val r = new GMP()
    this.bigInteger.shiftRight(n, r)
    new BigInt(r)
  }
  ...
  def &  (that: BigInt): BigInt = {
    val r = new GMP()
    this.bigInteger.and(that.bigInteger, r)
    new BigInt(r)
  }
  ...
  def |  (that: BigInt): BigInt = {
    val r = new GMP()
    this.bigInteger.or(that.bigInteger, r)
    new BigInt(r)
  }//new BigInt(this.bigInteger.or (that.bigInteger))

  /** Bitwise exclusive-or of BigInts
   */
  def ^  (that: BigInt): BigInt = {
    val r = new GMP()
    this.bigInteger.xor(that.bigInteger, r)
    new BigInt(r)
  ...
  def &~ (that: BigInt): BigInt = {
    val r = new GMP()
    this.bigInteger.andNot(that.bigInteger, r)
    new BigInt(r)
  ...
  def min (that: BigInt): BigInt = {
    if (this.bigInteger.compare(that.bigInteger) < 0)
      new BigInt(this.bigInteger)
    else
      new BigInt(that.bigInteger)
  ...
  def modPow (exp: BigInt, m: BigInt): BigInt ={
    val r = new GMP()
    this.bigInteger.modPow(exp.bigInteger, m.bigInteger, r)
    new BigInt(r)
  ...
  def modInverse (m: BigInt): BigInt = {
    val r = new GMP()
    this.bigInteger.modInverse(m.bigInteger, r)//(that.bigInteger, r)
    new BigInt(r)
  ...
  def unary_- : BigInt   = {
    val r = new GMP()
    this.bigInteger.negate(r)//(that.bigInteger, r)
    new BigInt(r)
  }//new BigInt(this.bigInteger.negate())
   ...
   */
  def signum: Int = this.bigInteger.compare(GMP.ZERO)
